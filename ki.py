...
#html crawling part
url = "http://www.espnfc.com/player/122608/ki-sung-yueng"
...
Result3 = ["W","D","L","w","d","l"]
if (dic["League"] == "Prem") and (dic["Result"][0] in Result3)  : # Only when latest game is part of the Premier League game, follow the code below.
    dic["League"] = "Epl"
    conn = sqlite3.connect("/home/ubuntu/epl/epl.db")
    cur = conn.cursor()
    query1 = "insert into ki(Date,League,Vs,Result,Goal,Assist,Sh,ShT,Yel,Red,Appearance) values(?,?,?,?,?,?,?,?,?,?,?)"
    
    cur.execute("select Date from ki order by id desc limit 1")
