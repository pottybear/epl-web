...
#html crawling part
url = "http://www.espnfc.com/player/237224/hwang-hee-chan"
...
Result3 = ["W","D","L","w","d","l"]
if (dic["League"] == "Bund 2") and (dic["Result"][0] in Result3)  : # Only when latest game is part of the Bundesliga game, follow the code below.
    conn = sqlite3.connect("/home/ubuntu/epl/epl.db")
    cur = conn.cursor()
    query1 = "insert into hhc(Date,League,Vs,Result,Goal,Assist,Sh,ShT,Yel,Red,Appearance) values(?,?,?,?,?,?,?,?,?,?,?)"
    
    cur.execute("select Date from hhc order by id desc limit 1")
