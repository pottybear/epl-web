...
url = "https://www.skysports.com/newcastle-united-results"
...
if UpdatedDate in [dbdate,dbdate_plus] :
    p = re.compile(r",\ski\s\((?P<Ratingobj>\d)\)",re.I) #Crawling Player's rating and MOM data contents with re module 

    try:
        rating = p.findall(source2.decode("utf-8"))[0]  
        cur.execute("update ki set rating = ? where date = ?" , (rating,dbdate))
        conn.commit()
        conn.close()
